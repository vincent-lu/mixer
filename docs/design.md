...
7. **Style-driven pacing** (`resolveMinGap` + `selectScoredBeatsBySection`) — `MixStyle` ('chill' | 'relaxed' | 'balanced' | 'energetic' | 'hyperkinetic' | 'frenetic' | 'chaos') × section energy ('low' | 'medium' | 'high') maps to a concrete minGap via a 7×3 table (12.0s for chill/low down to 0.12s for chaos/high). `selectScoredBeatsBySection` uses each beat's section energy to resolve minGap locally — tight cuts during drops, wide gaps during breakdowns. Per-style default lookahead controls the scoring window past the min gap: chill 1.0s, relaxed 0.8s, balanced 0.5s, energetic 0.3s, hyperkinetic 0.2s, frenetic 0.1s, chaos 0.0s. When lookahead is 0, the algorithm takes the first eligible beat (greedy, no scoring). Effective lookahead is `max(defaultLookahead, minGap × 0.4)`. Optional `lookahead` on `MixJobConfig` overrides the style default. Default style is 'balanced' when `MixStyle` is absent. Explicit `minSegmentDuration` overrides style-driven pacing entirely.
...
**Per-type durations** scaled by `MixStyle`: circleopen 0.6s, fadewhite 0.8s, horzopen/vertopen 0.6s, acid/doublevision 1.2s, solarize 1.0s, strobe/strobe_white 0.8s. Scale factors: chill ×1.5, relaxed ×1.2, balanced ×1.0, energetic ×0.7, hyperkinetic ×0.5, frenetic ×0.35, chaos ×0.2.
...
**CLI:** `pnpm mix --bgm <path> --videos <path...> --output <path> [--segment-duration <s> | --min-segment <s>] [--style <style>] [--lookahead <s>] [--transition-density 0-100] [--transition-effect <name>] [--clip-effect <name>] [--effect-chance 0-100] [--no-transitions]`
